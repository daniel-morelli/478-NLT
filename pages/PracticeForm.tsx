...
import { ArrowLeft, Save, Lock, User, History, Briefcase, ShieldCheck, ShoppingCart, Bell, Info, Search, UserPlus, Phone, Mail, AlertTriangle, X, Plus, Trash2, RefreshCw, Users, Layers, PhoneCall, Unlock, Target, Copy } from 'lucide-react';
...
export const PracticeForm: React.FC = () => {
  ...
  const duplicateVehicleCredit = (v: VehicleCredit) => {
    if (isPracticeLocked) return;
    const duplicated: VehicleCredit = { ...v, id: crypto.randomUUID() };
    setFormData(prev => ({ ...prev, veicoliAffidamento: [...(prev.veicoliAffidamento || []), duplicated] }));
  };
  ...
  const duplicateVehicleOrder = (v: VehicleOrder) => {
    if (isPracticeLocked) return;
    const duplicated: VehicleOrder = { ...v, id: crypto.randomUUID() };
    setFormData(prev => ({ ...prev, veicoliOrdine: [...(prev.veicoliOrdine || []), duplicated] }));
  };
