      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-gray-100">
                <tr><th className="px-6 py-3 font-semibold text-gray-600">Nome Provider</th><th className="px-6 py-3 font-semibold text-gray-600">Stato</th><th className="px-6 py-3 font-semibold text-gray-600 text-right">Azioni</th></tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
                {providers.map(p => (
                    <tr key={p.id} className="hover:bg-gray-50"><td className="px-6 py-4 font-medium text-gray-900 flex items-center gap-2"><Briefcase size={16} className="text-gray-400"/>{p.name}</td><td className="px-6 py-4">{p.isActive ? <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-green-100 text-green-700 text-xs font-bold"><CheckCircle size={12}/> Attivo</span> : <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-gray-100 text-gray-500 text-xs font-bold"><XCircle size={12}/> Disabilitato</span>}</td><td className="px-6 py-4 text-right space-x-2"><button onClick={() => handleEdit(p)} className="text-gray-600 hover:text-black font-medium text-sm transition-colors">Rinomina</button><button onClick={() => toggleActive(p)} className={`${p.isActive ? 'text-red-600 hover:text-red-800' : 'text-green-600 hover:text-green-800'} font-medium text-sm transition-colors`}>{p.isActive ? 'Disabilita' : 'Riattiva'}</button></td></tr>
                ))}
                {providers.length === 0 && (
                    <tr><td colSpan={3} className="px-6 py-8 text-center text-gray-400">Nessun provider inserito.</td></tr>
                )}
            </tbody>
        </table>
      </div>
